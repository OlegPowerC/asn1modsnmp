...
func parseBase128Int(bytes []byte, initOffset int) (ret, offset int, err error) {
	...
	for shifted := 0; offset < len(bytes); shifted++ {
		...
		if b&0x80 == 0 {
			...
			//Old Value math.MaxInt32 new value math.MaxUint32
...
func FindSNMPv3AuthParamsOffset(rawPacket []byte) (offset int, err error) {
	if len(rawPacket) < 10 {
		return -1, errors.New("packet too short")
	}
	...
	if err != nil {
		return -1, fmt.Errorf("failed to parse outer SEQUENCE: %v", err)
	}
	if t.class != ClassUniversal || t.tag != TagSequence {
		return -1, errors.New("expected SEQUENCE at start")
	}
	pos = newPos
	...
	if t.class != ClassUniversal || t.tag != TagOctetString {
		return -1, fmt.Errorf("expected OCTET STRING for msgSecurityParameters, got tag %d", t.tag)
	}
	pos = newPos

	// Внутри OCTET STRING - USM SEQUENCE
	t, newPos, err = parseTagAndLength(rawPacket, pos)
	if err != nil {
		return -1, err
	}
	if t.class != ClassUniversal || t.tag != TagSequence {
		return -1, errors.New("expected USM SEQUENCE inside msgSecurityParameters")
	}
	pos = newPos

	// 5. Внутри USM SEQUENCE идут поля по порядку:
	//    a) msgAuthoritativeEngineID (OCTET STRING)
	t, newPos, err = parseTagAndLength(rawPacket, pos)
	if err != nil {
		return -1, fmt.Errorf("failed to parse engineID: %v", err)
	}
	pos = newPos + t.length
	...
	t, newPos, err = parseTagAndLength(rawPacket, pos)
	if err != nil {
		return -1, fmt.Errorf("failed to parse boots: %v", err)
	}
	pos = newPos + t.length
	...
	t, newPos, err = parseTagAndLength(rawPacket, pos)
	if err != nil {
		return -1, fmt.Errorf("failed to parse time: %v", err)
	}
	pos = newPos + t.length
	...
	t, newPos, err = parseTagAndLength(rawPacket, pos)
	if err != nil {
		return -1, fmt.Errorf("failed to parse username: %v", err)
	}
	pos = newPos + t.length

	//    e) msgAuthenticationParameters (OCTET STRING) - ВОТ ОНО!
	t, newPos, err = parseTagAndLength(rawPacket, pos)
	if err != nil {
		return -1, fmt.Errorf("failed to parse authParams: %v", err)
	}

	if t.class != ClassUniversal || t.tag != TagOctetString {
		return -1, fmt.Errorf("expected OCTET STRING for authParams, got tag %d", t.tag)
	}

	if t.length != 12 {
		return -1, fmt.Errorf("authParams should be 12 bytes, got %d", t.length)
	}

	// newPos указывает на начало данных (после тега и длины)
	return newPos, nil
}
